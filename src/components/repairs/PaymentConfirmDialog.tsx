...
export function PaymentConfirmDialog({
  ...
  const debtAmount =
    paymentOption === "partial"
      ? remaining - (Number(partialAmount) || 0)
      : 0;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Avant de passer au statut "{statusLabel}", confirmez le paiement de cette réparation.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border bg-muted/50 p-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">{formatCurrency(repair.total)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>Déjà payé</span>
              <span className="font-medium">{formatCurrency(repair.paid)}</span>
            </div>
          ...
          <div className="space-y-3">
            ...
            <RadioGroup
              value={paymentOption}
              onValueChange={(value) => setPaymentOption(value as "full" | "partial")}
            >
              <div className="flex items-start space-x-3 p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Le client paie les {formatCurrency(remaining)} restants
                  </p>
                </div>
              </div>
                    ...
                    <div className="space-y-2 mt-2">
                      ...
                      {debtAmount > 0 && (
                        <p className="text-xs text-amber-600 dark:text-amber-500">
                          → {formatCurrency(debtAmount)} sera ajouté aux dettes du client
                        </p>
                      )}
                    </div>
                  )}
          ...
          {!hasCustomer && paymentOption === "partial" && debtAmount > 0 && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-400">
                Aucun client associé à cette réparation. La dette de {formatCurrency(debtAmount)} ne pourra pas être enregistrée.
              </p>
            </div>
          )}
        </div>
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          >
            {isLoading ? "Traitement..." : "Confirmer et changer"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
