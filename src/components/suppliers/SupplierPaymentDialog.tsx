...
export function SupplierPaymentDialog({ open, onOpenChange, supplier }: SupplierPaymentDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateBalance.mutateAsync({ id: supplier.id, amount: paymentAmount });
      setAmount("");
      onOpenChange(false);
    } catch (error) {
        ...
        {supplier && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="amount" type="number" step="0.001" min="0" max={currentDebt} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.000" required />
            </div>

            <div className="flex justify-end gap-3 pt-4">
              <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Annuler</Button>
              <Button type="submit" disabled={updateBalance.isPending}>Enregistrer</Button>
            </div>
          </form>
        )}
      </DialogContent>
